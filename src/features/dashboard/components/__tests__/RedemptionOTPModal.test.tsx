...
describe('RedemptionOTPModal', () => {
  it('renders modal with title and message when open', () => {
    renderWithProviders(
      <RedemptionOTPModal isOpen onClose={vi.fn()} onVerify={vi.fn()} />,
    )
  ...
  it('displays userPhone when provided', () => {
    renderWithProviders(
      <RedemptionOTPModal isOpen onClose={vi.fn()} onVerify={vi.fn()} userPhone="+233 12 345 6789" />,
    )
  ...
  it('calls onClose when Cancel is clicked', async () => {
    ...
    renderWithProviders(
      <RedemptionOTPModal isOpen onClose={onClose} onVerify={vi.fn()} />,
    )
  ...
  it('calls onVerify with OTP when form is submitted with 4 digits', async () => {
    ...
    const user = userEvent.setup()
    renderWithProviders(
      <RedemptionOTPModal isOpen onClose={vi.fn()} onVerify={onVerify} />,
    )
  ...
  it('does not render content when isOpen is false', () => {
    renderWithProviders(
      <RedemptionOTPModal isOpen={false} onClose={vi.fn()} onVerify={vi.fn()} />,
    )
