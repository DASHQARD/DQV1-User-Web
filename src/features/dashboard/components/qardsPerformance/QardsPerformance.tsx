...
function transformToChartData(
  ...
  const transformed = apiData.map((item) => {
    ...
    return {
      ...
      dashPro: item.dashpro_amount || 0,
      dashX: item.dashx_amount || 0,
      dashPass: item.dashpass_amount || 0,
      dashGo: item.dashgo_amount || 0,
    }
  })
  ...
  if (timeframe === 'Monthly') {
    ...
    const grouped = transformed.reduce(
      (acc, item) => {
        ...
        acc[key].dashPro += item.dashPro
        acc[key].dashX += item.dashX
        acc[key].dashPass += item.dashPass
        acc[key].dashGo += item.dashGo
        return acc
      },
      {} as Record<
        string,
  ...
  const grouped = transformed.reduce(
    ...
    {} as Record<
      string,
      { year: number; dashPro: number; dashX: number; dashPass: number; dashGo: number }
    >,
  )
...
const LEGEND_ITEMS = [
  ...
  { key: 'dashGo' as const, label: 'DashGo', color: '#0F8C5F' },
]
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} barCategoryGap="20%" barGap={8} stackOffset="sign">
              ...
              <Bar dataKey="dashPro" stackId="qards" fill="#FAC203" />
              <Bar dataKey="dashX" stackId="qards" fill="#402D87" />
              <Bar dataKey="dashPass" stackId="qards" fill="#ED186A" />
              <Bar dataKey="dashGo" stackId="qards" fill="#0F8C5F" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
